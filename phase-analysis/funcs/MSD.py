...
outputfilename = path_to_data + ".msd"
...
def calc_displacement_update_flags(lmp, ref):
    ...
    disp = np.zeros(
        shape=(lmp.nratoms, 1), dtype=float
    )  # assumes that lmp.nratoms doesnt change
...
def main_script():
    ...
    while 1:
        ...
        if lmp.image_flag.shape[0] == 0:
            dummy, dummy_x, dummy_y, dummy_z, imageflags = (
                calc_displacement_update_flags(lmp, lmp_ref)
            )
        else:
        ...
        for kk, lmp_orig in enumerate(refs):  # for all the start time traces...
            ...
            disp_sqr, disp_x, disp_y, disp_z = calc_displacement_use_flags(
                lmp, lmp_orig, imgflgs[kk]
            )
    ...
    for ii in range(MAX_NR_TYPES):
        fidout.write(
            " x^2_%d   y^2_%d   z^2_%d   msd^2_%d  " % (ii + 1, ii + 1, ii + 1, ii + 1)
        )
    fidout.write("\n")
